...
// build_runner에게 이전 build에서 만들어진 output과 이번 build에서 만들어질 output사이에
// 충돌이 발생하면 사용자에게 확인하는 과정을 거치지말고 build를 하라는 옵션입니다.
...
// 방금 말씀드린 part directive는 part이후에 파일이름 닷 g 닷 다트형태를 가지는데
...
// keepAlive: true 옵션은 riverpod generator에게 Provider를 generate할 때, autodispose modifier를 적용하지 말라는 옵션입니다.
// default는 false 입니다. 그러니까 default로는 Provider가 더 이상  필요 없어지면 즉, 더 이상 listen할 위젯이나 Provider가 없으면
// dispose된다는 의미입니다.
// 다음으로 function의 return 타입을 주겠습니다. 이전에 수동으로 Provider를 만들 때, 타입으로 String을 주었습니다.
// return 타입으로 String을 주겠습니다.
// 다음으로 function name을 주겠습니다.
// function 이름으로는 hello를 주겠습니다.
// function 이름으로 hello를 주면 generate된 Provider는 helloProvider라는 이름을 가지게 됩니다.
...
// riverpod generator는 이 function 이름 hello를 이용해서 helloProvider 변수를 만들어줍니다.

// 다음으로 수동방식에서는 사용하는 Provider의 종류를 명시했는데, code generation 방식에서는 사용하는 Provider의 종류를
// 지정하는 code가 없습니다. code generation 방식에서는 return value를 이용해서 사용하는 Provider의 종류를 riverpod generator가
// 지정해줍니다.

// 다음으로 수동방식에서는 state 타입을 제네릭을 이용해서 지정했었는데 code generation 방식에서는 annotate된 function의 return타입으로
// state 타입을 지정합니다.
...
// 맨 위에 보니 // GENERATED CODE - DO NOT MODIFY BY HAND라고 되어있습니다. 이 파일은 edit할 목적으로 만들어진 파일이 아니니 수정하지 마시기 바랍니다.
// 파일을 아래로 스크롤하니 helloProvider라는 final 변수가 보이네요
// 그리고 그 밑에 HelloRef typedef가 있는데 보니까 String타입의 ProviderRef를 typedef한 것임을 알 수 있습니다.
// 수동방식과 동일한 ref argument 입니다.

// 그런데 generator를 이용해서 만들어야할 Provider가 많거나 할 경우 매번 command line에서 명령을 실행시키기가 번거롭습니다.
// 이럴 경우를 대비해서 변화를 watch해서 변화가 있을 때, 자동적으로 build runner가 실행되게 할 수 있습니다.
// dart run build_runner build -d 대신에 dart run build_runner build watch -d를 실행시키면 됩니다.